...
A pinned step test stores one successful run’s input and expected output under `<Step>/tests/step-tests/`. Replays are hermetic, have no network egress, and compare output byte-for-byte except for trailing newlines. HTTP tests compare the complete response. Pin only deterministic behavior: do not pin output that depends on live data, the wall clock, or an unreplayable external effect.

When an edit reports existing pinned tests, run them after finishing the step. Run these tests without asking even if the normal step has side effects, because external requests are replayed and writable volumes are isolated. Never finish with a test failure unresolved.

Treat a failure according to its cause:

- If the edit introduced a bug, fix the code and rerun the tests.
- If intended behavior changed the expected output, show the user the expected-versus-actual result. Re-pin the same test only after the user confirms the new behavior.
- Unpin a test only when the user explicitly asks to remove that specific test.
...
Pinning records external requests and their responses for replay. If a request contains a timestamp, UUID, nonce, or another volatile field, add a deterministic POSIX `tests/serialize-request` script. It reads `{method, url, headers, body, bodyEncoding}` JSON from stdin, with `body` base64-encoded, and prints the stable key used to match that request during replay.
