...
Follow the `building-workflows` skill’s **Route URLs** guidance for links and callbacks generated by the workflow. Configure external MCP clients with the destination route’s URL after import.
...
Step code also receives verified group names in the spoof-proof `x-3b-authenticated-groups` header as a JSON array of strings. Parse it as JSON, never split on commas. It contains the IdP group snapshot for SSO callers and current 3B group names for members, and is absent when there are no verified groups. 3B strips caller-supplied values before injecting verified names.

To restrict an `sso` route to certain IdP groups, add `route_sso_groups = ["Security Team"]`; 3B requires membership in at least one listed group, compared case-insensitively, before the step runs. Other auth modes reject this setting. Draft previews require space access and do not enforce the SSO group restriction. The checks use the IdP groups captured at sign-in. That snapshot survives the fixed 30-day OAuth refresh chain, with up to 30 additional minutes for the final access token; refreshing does not pick up group changes. The IdP must emit the `groups` claim, and missing or oversized claims cannot satisfy a group restriction.
...
- `x-3b-consent-decision-url` — where to send the decision; use it unchanged rather than constructing a workflow route URL.
